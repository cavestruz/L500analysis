...
halo_properties_list=['r200c','M_total_200c']


fTz200c=r"$\tilde{T}_{200c}/\tilde{T}(z=0)$"
fTz500c=r"$\tilde{T}_{500c}/\tilde{T}(z=0)$"
fTz1600c=r"$\tilde{T}_{1600c}/\tilde{T}(z=0)$"
axes_labels = [fTz200c,fTz500c,fTz1600c]
text_xlocs = [.125,0.2,0.2]
pa = PlotAxes(figname='fTmw_deltac',use_axes_labels=False,
              axes=[[0.15,0.65,0.80,0.2],[0.15,0.4,0.80,0.2],[0.15,0.15,0.80,0.2]],
              axes_labels=axes_labels,
              xlabel=r"$R/R_{\Delta}$",
              xlims=[(0.06,0.94),(0.1,1.39),(0.01,2.49)],
              ylims=[(0.8,1.1),(0.8,1.1),(0.8,1.1)])
fT1 = {}
fT2 = {}
fT3 = {}
Tplots = [fT1, fT2, fT3]
deltas = ['200c','500c','1600c']
...
for axes_label,delta,xloc in zip(axes_labels,deltas,text_xlocs) :
    ...
    pa.axes[axes_label].set_yticks(arange(0.8,1.1,0.1))
    pa.axes[axes_label].text(xloc,1.03,'$\\Delta='+delta+'$')

pa.axes[axes_labels[1]].set_ylabel("$\\tilde{T}_{\\Delta}/\\tilde{T}(z=0)$",
                                   fontsize="xx-large")
pa.set_legend(axes_label=axes_labels[0],ncol=3,loc='lower left', frameon=False)
