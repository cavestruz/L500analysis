...
pa = PlotAxes(figname='fTtot_deltam',use_axes_labels=False,
              axes=[[0.15,0.65,0.80,0.2],[0.15,0.4,0.80,0.2],[0.15,0.15,0.80,0.2]],
              axes_labels=axes_labels,
              xlabel=r"$R/R_{\Delta}$",
              xlims=[(0.06,0.94),(0.1,1.39),(0.01,2.49)],
              ylims=[(0.8,1.3),(0.8,1.3),(0.8,1.3)])
fT1 = {}
fT2 = {}
fT3 = {}
Tplots = [fT1, fT2, fT3]
deltas = ['200m','500m','1600m']
clkeys = ['T_tot/T'+delta for delta in deltas]
linestyles = ['-']
...
for axes_label,delta,xloc in zip(axes_labels,deltas,text_xlocs) :
    ...
    pa.axes[axes_label].set_yticks(arange(0.8,1.3,0.2))
    pa.axes[axes_label].text(xloc,1.2,'$\\Delta='+delta+'$')

pa.axes[axes_labels[1]].set_ylabel("$\\tilde{T}_{tot}/\\tilde{T}_{tot}(z=0)$",
                                   fontsize="xx-large")
