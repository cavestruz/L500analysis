...
nu_threshold = [2, 2.5] # 1.3, 2, 2.5, 3.6
...
profiles_list = ['T_mw', 'r_mid', 
                 ...
                 'M_dark', 'M_star', 'M_gas',
                 'Tnt_Vcirc2_ratio_200m',
                 'R/R200m']

halo_properties_list=['r200m','M_total_200m','nu_200m']


Tnt_Vcirc2_ratio=r"$\Xi=T_{nt}/V^2_{circ}$"
fXz1=r"$\Xi/\Xi(z=1)$"

pa = PlotAxes(figname='Tnt_Vcirc2_ratio_200m_nu%0.1f'%nu_threshold[0],
              axes=[[0.15,0.4,0.80,0.55],[0.15,0.15,0.80,0.24]],
              axes_labels=[Tnt_Vcirc2_ratio,fXz1],
              xlabel=r"$R/R_{200m}$",
              xlim=(0.2,2),
              ylims=[(0.,0.8),(0.6,1.4)])
...
for aexp in aexps :
    ...
    pa.axes[Tnt_Vcirc2_ratio].plot( rbins, TratioV2[aexp]['mean'],
                                     color=color(aexp),ls='-',
                                     label="$z=%3.1f$" % aexp2redshift(aexp))


pa.axes[Tnt_Vcirc2_ratio].fill_between(rbins, TratioV2[0.5]['down'], 
                                        TratioV2[0.5]['up'], 
...
pa.axes[Tnt_Vcirc2_ratio].annotate(nu_label, xy=(.75, .75), xytext=(.3,.7),
                         )
pa.axes[Tnt_Vcirc2_ratio].tick_params(labelsize=12)
pa.axes[Tnt_Vcirc2_ratio].tick_params(labelsize=12)
pa.axes[fXz1].set_yticks(arange(0.6,1.4,0.2))
pa.set_legend(axes_label=Tnt_Vcirc2_ratio,ncol=3,loc='best', frameon=False)
pa.color_legend_texts(axes_label=Tnt_Vcirc2_ratio)
